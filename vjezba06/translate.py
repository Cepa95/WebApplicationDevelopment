...
def print_subjects(row, data):
    ...
    if (data.get(row[0])==None):
        print('<input type="radio" name="' + str(row[0]) + '" value=''not'' checked>' + predmeti.status_names.get('not'))
        print('<input type="radio" name="' + str(row[0]) + '" value=''enr''>' + predmeti.status_names.get('enr'))
        print('<input type="radio" name="' + str(row[0]) + '" value=''pass''>' + predmeti.status_names.get('pass'))
    else:
         for status_key, value in predmeti.status_names.items():  #provjera je li korisnik odabrao polje, defaultno je 'not'

                if data[row[0]] == status_key:
                        print(value + '<input type="radio" name="' + str(row[0]) + '" value="' + status_key + '" checked/>') #checkiranje
                else:
                        print(value + '<input type="radio" name="' + str(row[0]) + '" value="' + status_key + '" >')
    print('''
        </td>
    </tr>
    ''')
...
def print_list_button(subjects,data):
    ...
    for row in subjects:
        ...
        print('''
                    <tr>
                        <td>''' + row[2] + '''</td>
                        <td>''' + predmeti.status_names.get(data[row[0]]) + '''</td>
                        <td>''' + str(row[3]) + '''</td> 
                    </tr>''')       
    print('''
    <tr>
